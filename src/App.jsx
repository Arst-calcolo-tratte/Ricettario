...
const FONTS = (
  <style>{`
    @import url('https://fonts.googleapis.com/css2?family=Fraunces:ital,opsz,wght@0,9..144,500;0,9..144,700;1,9..144,500&family=Inter:wght@400;500;600;700&display=swap');
    .rc-display { font-family: 'Fraunces', serif; }
    .rc-display-i { font-family: 'Fraunces', serif; font-style: italic; }
    .rc-sans { font-family: 'Inter', sans-serif; }
...
function Plate({ fill = "#EFE8D8" }) { return <><ellipse cx="70" cy="68" rx="50" ry="21" fill={PALETTE.ink} opacity="0.15" /><ellipse cx="70" cy="64" rx="48" ry="20" fill={fill} stroke={PALETTE.ink} strokeWidth="3" /></>; }

/* ---- Primi ---- */
function PastaArt() {
  return (<svg viewBox="0 0 140 100" width="100%" height="100%" preserveAspectRatio="xMidYMid slice"><rect width="140" height="100" fill={PALETTE.tomato} /><NapkinCorner color={PALETTE.saffron} /><Plate fill={PALETTE.card} /><path d="M40 58 Q46 48 58 50 Q64 42 74 48 Q68 56 58 58 Q50 62 40 58 Z" fill={PALETTE.dough} stroke={PALETTE.ink} strokeWidth="2.4" strokeLinejoin="round" /><g transform="translate(28,8)"><path d="M40 58 Q46 48 58 50 Q64 42 74 48 Q68 56 58 58 Q50 62 40 58 Z" fill={PALETTE.dough} stroke={PALETTE.ink} strokeWidth="2.4" strokeLinejoin="round" /></g><g transform="translate(-16,18)"><path d="M40 58 Q46 48 58 50 Q64 42 74 48 Q68 56 58 58 Q50 62 40 58 Z" fill={PALETTE.dough} stroke={PALETTE.ink} strokeWidth="2.4" strokeLinejoin="round" /></g><rect className="rc-steam" x="55" y="6" width="5" height="20" rx="2.5" fill="#fff" opacity="0.7" /></svg>);
}
function SeafoodArt() {
...
function TagPill({ tag, i }) {
  const colors = [PALETTE.tomatoSoft, PALETTE.basilSoft, PALETTE.saffronSoft, PALETTE.plumSoft];
  const textColors = [PALETTE.tomatoDeep, PALETTE.basilDeep, PALETTE.saffronDeep, PALETTE.plum];
  return <span className="rc-sans" style={{ fontSize: 11, color: textColors[i % 4], background: colors[i % 4], borderRadius: 20, padding: "3px 10px", fontWeight: 700, transform: `rotate(${i % 2 === 0 ? -2 : 2}deg)`, display: "inline-block" }}>{tag}</span>;
}
...
function FeaturedCard({ recipe, onOpen }) {
  ...
  return (
    <button onClick={() => onOpen(recipe)} className="rc-sans rc-card rc-fadeup" style={{ width: "100%", textAlign: "left", background: PALETTE.card, border: `3px solid ${PALETTE.ink}`, borderRadius: 22, overflow: "hidden", cursor: "pointer", padding: 0, boxShadow: `5px 6px 0 ${color}`, marginBottom: 26, transform: "rotate(-0.6deg)" }}>
...
function RecipeCard({ recipe, onOpen, idx }) {
  ...
  return (
    <button onClick={() => onOpen(recipe)} className="rc-sans rc-card rc-fadeup" style={{ display: "flex", alignItems: "stretch", width: "100%", textAlign: "left", background: PALETTE.card, border: `2.5px solid ${PALETTE.ink}`, borderRadius: 16, overflow: "hidden", cursor: "pointer", padding: 0, boxShadow: `4px 4px 0 ${color}`, transform: `rotate(${rot}deg)`, animationDelay: `${idx * 0.06 + 0.1}s` }}>
      ...
      <div style={{ display: "flex", alignItems: "center", paddingRight: 14, color }}><Icon name="chevronRight" size={20} /></div>
    </button>
  );
}
...
export default function App() {
  ...
  useEffect(() => {
    ...
    async function loadMessages() {
      ...
      setMessages((data || []).map((m) => ({ id: m.id, from: m.sender === "me" ? "me" : "them", text: m.text, time: new Date(m.created_at).toLocaleString("it-IT", { hour: "2-digit", minute: "2-digit" }) })));
    }
    loadMessages();
    ...
    const channel = supabase
      ...
      .on("postgres_changes", { event: "INSERT", schema: "public", table: "messages" }, (payload) => {
        const m = payload.new;
        setMessages((prev) => (prev.some((x) => x.id === m.id) ? prev : [...prev, { id: m.id, from: m.sender === "me" ? "me" : "them", text: m.text, time: new Date(m.created_at).toLocaleString("it-IT", { hour: "2-digit", minute: "2-digit" }) }]));
      })
      .subscribe();
  ...
  async function sendMessage(text) {
    await supabase.from("messages").insert({ sender: "me", text });
  }
  ...
  return (
    <div style={{ background: PALETTE.bg, minHeight: 500, borderRadius: 24, padding: "18px 18px 0", maxWidth: 380, margin: "0 auto", position: "relative", overflow: "hidden" }}>
      ...
      <div style={{ position: "relative" }}>
        ...
        {tab === "chat" && view === "list" && (
          <>
            <h1 className="rc-display" style={{ fontSize: 27, fontWeight: 700, color: PALETTE.ink, margin: "10px 0 18px" }}>Chat</h1>
